...
export const GET = withApiErrorHandling(async (_request: NextRequest, context: Context) => {
  ...
  const item = await new UnitOfWork().destinations.findOne({ slug });
  if (!item) throw new NotFoundError('Destination not found');
  return ok(item);
});
...
export const PUT = withApiErrorHandling(async (request: NextRequest, context: Context) => {
  ...
  const item = await new UnitOfWork().destinations.updateOne({ slug }, data);
  if (!item) throw new NotFoundError('Destination not found');
  return ok(item);
});
...
export const DELETE = withApiErrorHandling(async (request: NextRequest, context: Context) => {
  await requireApiUser(request, AdminRoles);
  const { slug } = await context.params;
  const deleted = await new UnitOfWork().destinations.deleteOne({ slug });
  if (!deleted) throw new NotFoundError('Destination not found');
  return noContent();
});
