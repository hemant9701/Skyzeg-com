...
import { TravelTypeUpsertSchema } from '@/application/validators/content.validators';
...
export const GET = withApiErrorHandling(async (_request: NextRequest, context: Context) => {
  ...
  const item = await new UnitOfWork().travelTypes.findOne({ slug });
  if (!item) throw new NotFoundError('Travel type not found');
  return ok(item);
});
...
export const PUT = withApiErrorHandling(async (request: NextRequest, context: Context) => {
  ...
  const { slug } = await context.params;
  const data = TravelTypeUpsertSchema.parse(await request.json());
  const item = await new UnitOfWork().travelTypes.updateOne({ slug }, data);
  if (!item) throw new NotFoundError('Travel type not found');
  return ok(item);
});
...
export const DELETE = withApiErrorHandling(async (request: NextRequest, context: Context) => {
  await requireApiUser(request, AdminRoles);
  const { slug } = await context.params;
  const deleted = await new UnitOfWork().travelTypes.deleteOne({ slug });
  if (!deleted) throw new NotFoundError('Travel type not found');
  return noContent();
});
