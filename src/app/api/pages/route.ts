...
import { PageUpsertSchema } from '@/application/validators/content.validators';
...
export const GET = withApiErrorHandling(async (request: NextRequest) => {
  ...
  const result = admin
    ? await uow.pages.paginate(filter, { page, pageSize, sort: { createdAt: -1 } })
    : await uow.pages.list(filter, { sort: { sortOrder: 1, createdAt: -1 } });
  return ok(result);
});

export const POST = withApiErrorHandling(async (request: NextRequest) => {
  await requireApiUser(request, AdminRoles);
  const data = PageUpsertSchema.parse(await request.json());
  const createdItem = await new UnitOfWork().pages.create(data);
