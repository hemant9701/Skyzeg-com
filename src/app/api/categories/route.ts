...
export const GET = withApiErrorHandling(async (request: NextRequest) => {
  ...
  const result = admin
    ? await uow.categories.paginate(filter, { page, pageSize, sort: { createdAt: -1 } })
    : await uow.categories.list(filter, { sort: { sortOrder: 1, createdAt: -1 } });
  return ok(result);
});
...
export const POST = withApiErrorHandling(async (request: NextRequest) => {
  ...
  const createdItem = await new UnitOfWork().categories.create(data);
