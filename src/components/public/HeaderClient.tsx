...
export default function HeaderClient({ logoUrl, logo2Url, siteName, menuItems, languages, activeLanguage }: HeaderClientProps) {
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-line bg-white/95 shadow-sm backdrop-blur">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
        ...
        <button
          type="button"
          className="inline-flex items-center rounded-full border border-line p-2 text-body md:hidden"
        ...
        <div className="hidden items-center gap-2 md:flex">
          <ul className="flex items-center gap-1">
            {items.map((item) => (
              <li key={String(item._id || item.url)}>
                <Link
                  className="rounded-full px-3 py-2 text-sm font-medium text-body transition hover:bg-light hover:text-dark"
                  href={item.url}
          ...
          <div className="ml-1 flex items-center gap-2">
            ...
            <Link className="rounded-full bg-primary px-4 py-2 text-md font-medium text-white transition hover:bg-primary-hover" href="/tailor-made">
              {ui.navBookNow}
            </Link>
          </div>
        </div>
        ...
        <div className="border-t border-line bg-white px-4 py-4 md:hidden">
          <ul className="flex flex-col gap-1">
            {items.map((item) => (
              <li key={String(item._id || item.url)}>
                <Link
                  className="block rounded-2xl px-3 py-2 text-sm font-medium text-body transition hover:bg-light hover:text-dark"
                  href={item.url}
                  target={item.target || '_self'}
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
            <LanguageSwitcher languages={languages} active={activeLanguage} />
            <Link className="rounded-full bg-primary px-4 py-2 text-sm font-semibold text-white transition hover:bg-primary-hover" href="/tailor-made" onClick={() => setMobileMenuOpen(false)}>
              {ui.navBookNow}
            </Link>
          </div>
        </div>
