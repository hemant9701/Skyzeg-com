...
const inputClass='w-full rounded-xl border border-white/10 bg-slate-950/60 px-3 py-2.5 text-slate-100 outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/10';
const smallInputClass='w-full rounded-lg border border-white/10 bg-slate-950/60 px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/10';
...
export default function AdminContentManager({config}:{config:AdminModuleConfig}){
 ...
 return <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-4 shadow-2xl sm:p-6">
  <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><h1 className="text-2xl font-bold text-white">{config.title}</h1>{config.description&&<p className="mt-1 max-w-3xl text-sm text-slate-400">{config.description}</p>}</div><button onClick={newItem} className="rounded-full bg-primary px-5 py-2.5 font-semibold text-white shadow-lg shadow-dark/30 hover:-translate-y-0.5 hover:bg-primary-hover flex items-center gap-2 justify-center"><Plus size={18} /> Create</button></div>
...
function FooterColumnsRepeater({value,onChange}:{value:any[];onChange:(v:any[])=>void}){
 ...
 const remove=(i:number)=>onChange(renumber(value.filter((_,idx)=>idx!==i)));
 const move=(i:number,dir:number)=>moveItem(value,i,dir,next=>onChange(renumber(next)));
 return <div className="space-y-4">{value.map((column,i)=><div key={i} className={panelClass}><RepeaterHeader title={`Footer column ${i+1}`} onMoveUp={()=>move(i,-1)} onMoveDown={()=>move(i,1)} onRemove={()=>remove(i)} disableUp={i===0} disableDown={i===value.length-1}/><div className="grid gap-4 md:grid-cols-2"><LabeledInput label="Column Title" value={column.title||''} onChange={next=>update(i,'title',next)}/><LabeledInput label="Sort Order" type="number" value={column.sortOrder??i} onChange={next=>update(i,'sortOrder',Number(next||0))}/><div className="md:col-span-2"><FooterLinksRepeater value={Array.isArray(column.links)?column.links:[]} onChange={links=>update(i,'links',links)}/></div></div></div>)}<button type="button" onClick={add} className="w-full rounded-xl border border-dashed border-accent/50 px-4 py-3 font-medium text-accent hover:bg-primary/10">+ Add footer column</button></div>
}
...
function DynamicSelectField({field,value,onChange}:{field:AdminFieldConfig;value:any;onChange:(v:any)=>void}){
 ...
 if(field.type==='multiselect'){
  ...
  return <div className="space-y-2 rounded-xl border border-white/10 bg-slate-950/60 p-3 max-h-52 overflow-y-auto">{!loaded&&<span className="text-xs text-slate-400">Loading...</span>}{options.map(opt=><label key={opt.value} className="flex items-center gap-2 cursor-pointer"><input type="checkbox" className="h-4 w-4 accent-primary" checked={selected.includes(opt.value)} onChange={()=>toggle(opt.value)}/><span className="text-sm text-slate-300">{opt.label}</span></label>)}</div>;
 }
...
function Field({field,value,onChange,languages}:{field:AdminFieldConfig;value:any;onChange:(v:any)=>void;languages:{code:string;name:string}[]}){
 if(field.type==='checkbox')return <label className="inline-flex cursor-pointer items-center gap-3"><input type="checkbox" className="h-5 w-5 accent-primary" checked={Boolean(value)} onChange={e=>onChange(e.target.checked)}/><span className="text-sm text-slate-400">Enabled</span></label>;
